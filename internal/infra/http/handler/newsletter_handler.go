...
func NewNewsletterHandler(sessionService session.SessionService, meowService *meow.MeowServiceImpl) *NewsletterHandler {
...
// @Success 200 {object} types.NewsletterResponse
...
func (h *NewsletterHandler) ListNewsletters(c *gin.Context) {
	...
	if err != nil {
		...
		utils.RespondWithError(c, http.StatusInternalServerError, "Failed to get newsletters", err.Error())
		return
	}
	...
	for _, newsletter := range newsletters {
		...
		newsletterList = append(newsletterList, types.NewsletterMetadata{
	...
	response := types.NewsletterResponse{
