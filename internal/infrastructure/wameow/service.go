...
func (m *MeowService) GetClientStatus(sessionID string) types.Status {
	client := m.getClient(sessionID)
	if client == nil {
...
func (m *MeowService) ConnectOnStartup(ctx context.Context) error {
	...
	for _, sessionEntity := range sessions {
		...
		m.logger.Infof("Attempting to reconnect session %s (status: %s, wa_jid: %s)",
			sessionEntity.ID, sessionEntity.Status, sessionEntity.WaJID)

		if err := m.StartClient(sessionEntity.ID); err != nil {
