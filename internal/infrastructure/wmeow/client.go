...
func (c *Client) GetQRCodeBase64() (string, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *Client) PairPhone(phoneNumber string) (string, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) GetStatus() types.Status {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.status
}
...
func (c *Client) GetJID() waTypes.JID {
